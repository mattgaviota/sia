...
class DButils(object):
    ...
    def set_pass(self):
        os.putenv('PGPASSWORD', self.data.get_pass())
...
class Handler(object):
    ...
    def __init__(self, request):
        ...
        self.destino = DButils(
            host=request['ip_dest'],
            port=request['port_dest'],
            dbname=request['db_dest']
        )
        self.hospital = request['hospital']
        try:
            self.clean_db = request['clean_db']
        except KeyError:
            self.clean_db = False
